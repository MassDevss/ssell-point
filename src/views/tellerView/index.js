...
fetch("../precios.json")
  	...
  	.then((data) => {
    	for (let i = 0; i < data.length; i++) {
			...
			if (!sections.includes(elem.type)){
				sections.push(elem.type);
			}
...
function createTicket(isCopy) {
	...
	dataPrint.push(
		{
			type: 'text',
			value: `num: ${numPedido}      ${myFecha}`,
			style: {fontFamily: "Arial", marginBottom: "20px", marginTop: "100px"}
		}
	)
	...
	for (let i = 0; i < orderProducts.length; i++) {
		...
		dataOrder.total += parseInt(product[1]);
	...
	window.mainView.print(dataPrint, dataOrder);
}
